  ...
  if ($admin_id) {
    ...
    if (isset($_POST['status']) && isset($_POST['group_id']) && isset($_POST['date'])) {
      ...
      if ($group_id && $date && $status) {
        ...
        $q1 = "SELECT * from manage_queue where group_id = '$group_id'";
        ...
        if ($q1_run = $db->query($q1)) {
          ...
          if ($q1_run->num_rows) {
            ...
            if ($status == "Delivered") {
              ...
              $q = "UPDATE my_orders SET deliver_date = '$date' where group_id = '$group_id'";
            ...
            $q2 = "UPDATE manage_queue SET delivery_status = '$status', deliver_date = '$date' where group_id = '$group_id'";

            if ($q2_run = $db->query($q2)) {

              $q3 = "UPDATE my_orders SET status = '$status' where group_id = '$group_id'";
                    ...
                    if ($q6_run = $db->query($q6)) {
                      ...
                      if ($q6_run->num_rows) {
                        ...
                        if ($q6_fetch = $q6_run->fetch_assoc()) {
                          ...
                          array_push($arr, "Order was marked delivered on $date by Admin-Email-ID: $admin_email and Admin-ID: $admin_id");
